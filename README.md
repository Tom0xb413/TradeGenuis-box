...
- **共享扫描进度**：全市场 / 快扫 / 币圈 / 自选池扫描把结构化进度写入服务端 `scan_progress`；所有打开中的看板每秒轮询 `GET /api/status`，顶部显示同一条进度条与百分比。进页时若服务器已在扫，无需点击也会自动出现进度条；扫描进行中再点扫描会 **加入当前任务**（`status: running`），不会开第二轮
- **扫描并发**：默认 **16** 线程（`scan_workers`，钳制 4–32），自选池也已并行。上游限流报错增多时可把并发降到 8 或 4（见下）
- **实时行情刷新**：达标标的每 3 秒静默刷新价格/涨跌
- **K 线懒加载**：卡片进入视口附近才请求 `/api/kline` 并绘制（IntersectionObserver），同时最多 4 路并发，避免进页时按卡片数打满上游；币圈仍列出全部进池标的，优先加载视口内卡片，达标卡在队列中优先
...
- **本 fork 增强**（相对上游）：
  ...
  - 币圈：Binance 主源，失败后**粘性**回退 Gate.io USDT 永续（`BTC_USDT` → `BTCUSDT`）
  - 东财 push2 全挂时经 AKShare 走新浪概念/交易所官方名单/资金流等非 push2 接口，扫描尽量完成而非中止
  - 看板「全市场扫描 / 币圈扫描」默认使用 **1 小时结果缓存**；「强制重扫」或交易日 11:30/15:00 调度会绕过缓存
...
自选池、全市场、币圈扫描共用同一套线程池大小，默认 **16**，允许范围 **4–32**。
