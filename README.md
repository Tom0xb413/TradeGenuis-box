...
- **全市场标的扫描**：USDT 永续涨幅前 **20** + 黄金（Gate `XAUT_USDT`）+ 美/日/韩指数与龙头。有 Gate **股票代币** 的标的走原生 4h/8h/1d（优先现货 xStock `AAPLX_USDT`，否则永续干净名；跟踪正股、有基差，非交易所官方行情）；无代币的美股走新浪分钟K，日韩正股走 Sina/Naver 日K。覆盖池可点顶栏「覆盖」编辑。详见 [docs/crypto-global-pool.md](docs/crypto-global-pool.md)
