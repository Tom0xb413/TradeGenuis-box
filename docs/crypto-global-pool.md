...
常量集中在 [`global_pool.py`](../global_pool.py)（含 `GATE_EQUITY_MAP`），无代币时的日K 在 [`equity_sources.py`](../equity_sources.py)。

## 股票代币（重要）

美/日/韩 **有 Gate USDT 永续合约** 的标的，主路径走 **Gate 股票代币**（`AAPL_USDT`、`SPX500_USDT`、`TM_USDT` 等）。这与币圈一样有原生 **4h / 8h / 1d** K 线，国内 VPS 可直连。

代币**跟踪正股但存在基差**，看板徽章为「美股代币 / 日股代币 / …」，**不是**纽交所、东证、韩交所的官方打印。

无对应代币时才回退 Sina / Naver **日K**，4h/8h 带 `interval_note`。

映射表见 `GATE_EQUITY_MAP`（2026-09 本环境实测）：
...
| `.NDX` | `NAS100_USDT` | |
| `.IXIC` | （无） | 纳指综合，Sina 日K |
| `7203.T` | `TM_USDT` | 丰田 ADR 代币，与日元正股标尺不同 |
| `6758.T` | `SONY_USDT` | |
| `005930` | `SAMSUNG_USDT` | |
| `000660` | `SKHYNIX_USDT` | |
| `NKY` / KOSPI / KOSDAQ | （无） | Sina / Naver 日K。`JPN225_USDT` 报价约 427，与日经点数不是同一标尺，故不映射 |

覆盖池额外可解析 `SPY` / `QQQ`（`GATE_EXTRA_EQUITY`），不进默认宇宙。

## 宇宙（默认混合池）

| 类别 | `asset_class` | 数量 | 主源 |
|---|---|---|---|
| USDT 永续涨幅榜 | `crypto` | `CRYPTO_TOP_N = 20` | Binance `fapi` 24h ticker，失败后**粘性** Gate.io；股票代币合约不占 TOP20 |
| 黄金（1 只） | `gold` | 1 | Gate **`XAUT_USDT`**（不要用 `XAU_USDT` 当首选）→ `XAUUSDT` / `PAXGUSDT` |
| 美股指数 | `us_index` | 4 | Gate 代币（标普/道指/纳指100）或新浪 JSONP（纳指综合） |
| 美股大盘 | `us_stock` | `US_STOCKS` 约 20 | Gate `{SYM}_USDT`；无合约再新浪 JSONP |
| 日经 225 | `jp_index` | 1 | 新浪 `gi.finance.sina.com.cn/hq/daily?symbol=NKY` |
| 日股龙头 | `jp_stock` | 2 | Gate `TM_USDT` / `SONY_USDT`，失败再 Naver |
...
- 杠杆/反向代币（`AAPLX`、`3L`/`3S`、CSOP 2L 等）不当作默认映射
...
| 值 | 币 / 金 / **股票代币**（Gate 原生） | 无代币的股票 / 指数 |
|---|---|---|
| `4h` / `8h` | 原生周期 | **无稳定多日分钟历史** → 回退 **日K**，并带 `interval_note` |
| `1d` | 日K | 日K（新浪 / Naver） |

股票代币 K 线**不**走 Binance（避免 AAPL 打币所失败后把整轮粘性切到 Gate）。使用 `fetch_gate_equity_klines`。
...
- **空**：默认混合池
- 校验：Gate 股票代币合约名或正股别名（`AAPL` / `AAPL_USDT` / `TM` / `SAMSUNG`）→ 永续 ticker / 黄金 / Sina / Naver
...
- Gate 代币 K 线失败：回退 Sina/Naver 日K
